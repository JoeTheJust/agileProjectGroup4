...
void DBInterface::EnterAttendance(int sectionID, int studentId, string date, string attendanceStatus) {
    string sql = "INSERT INTO Attendance (sectionID, studentId, date, status) VALUES (?, ?, ?, ?);";
    ...
    if(status == SQLITE_OK) {
        sqlite3_bind_int(myStatement, 1, sectionID);
...
void DBInterface::CreateAttendanceTable() {
    string sql = "CREATE TABLE IF NOT EXISTS Attendance ( sectionId INTEGER NOT NULL, studentID INTEGER NOT NULL, date TEXT, status TEXT, PRIMARY KEY(sectionId, studentId), FOREIGN KEY (sectionId) REFERENCES Sections(id), FOREIGN KEY (studentID) REFERENCES Students(id));";
